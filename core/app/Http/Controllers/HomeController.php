...
class HomeController extends Controller
{
    ...
    public function HomePageByLang($lang = "")
    {
        ...
        $sectionid = 2;
        $url_query = str_ireplace(url('/'), '', url()->full());

        // Get Home page Test banners
        if($url_query == '' || $url_query == '/' || $url_query == '/home') {
            $sectionid = 2;
        }
        if($url_query == '/about-us') {
            $sectionid = 4;
            $HomePhotos = [];
        }
        if($url_query == '/contact-us') {
            $sectionid = 6;
            $HomePhotos = [];
            $HomePartners = [];
        }
        if($url_query == '/location') {
            $sectionid = 7;
            $HomePhotos = [];
            $HomePartners = [];
        }
        if($url_query == '/imprint') {
            $sectionid = 8;
            $HomePhotos = [];
            $HomePartners = [];
        }
        if($url_query == '/data-preferences') {
            $sectionid = 9;
            $HomePhotos = [];
            $HomePartners = [];
        }
        ...
        if($url_query == '/coming-soon') {
            $sectionid = 11;
            $HomePhotos = [];
            $HomePartners = [];
        }
        if($url_query == '/request-partnership') {
            $sectionid = 11;
            $HomePhotos = [];
            $HomePartners = [];
        }
        if($url_query == '/tickets') {
            $sectionid = 11;
            $HomePhotos = [];
            $HomePartners = [];
        }
        if($url_query == '/merchandise') {
            $sectionid = 11;
            $HomePhotos = [];
            $HomePartners = [];
        }
        if($url_query == '/ibanksy') {
            $sectionid = 11;
            $HomePhotos = [];
            $HomePartners = [];
        }
        ...
        $TextBanners = Banner::where('status', 1)->where('section_id', $sectionid)->orderby('row_no', 'asc')->get();
        ...
        $url_query = strtolower(str_ireplace('/', '', $url_query));
    ...
    public function subscribeSubmit(Request $request)
    {
        ...
        if (count($Contacts) > 0) {
            ...
            $Contact->message = $request->subscribe_message;
            $Contact->status = 1;
            $Contact->save();
