@if(Helper::GeneralWebmasterSettings("header_menu_id") >0)
    ...
    @if(count($HeaderMenuLinks)>0)
        ...
        ?>
    @desktop
        <div class="" style="display: grid;margin: 0 auto;">
            <ul class="nav navbar-nav pull-left" style="width:80%;display:flex;height:26px;margin-left: 5%">
    @elsedesktop
    ...
    @enddesktop
                @foreach($HeaderMenuLinks as $HeaderMenuLink)
                    ...
                    ?>
                    @if($HeaderMenuLink->type > 3) @continue @endif
                    ...
                    @elseif($HeaderMenuLink->type==1)
                        ...
                        ?>
                        <li class="item"><a href="{{ $this_link_url }}">{{ $link_title }}</a></li>
                    @else
                        <?php
                        ...
                        <li class="item dropdown">
                            ...
                            @if(count($HeaderMenuLink->subMenus) >0)
                                <ul class="dropdown-menu" style="margin-top:16px;margin-left: 10px;background-color:#000">
                                    @foreach($HeaderMenuLink->subMenus as $subMenu)
                                        ...
                                        @elseif($subMenu->type==1)
                                            ...
                                            <li class="item sub-item">
                                                <a href="{{ $this_link_url }}">{{ $subMenu_title }}</a>
                                            </li>
                                        @else
                                            <?php
                                            // sub menu - Main title ( have drop down menu )
                                            ?>
                                            <li class="item"><a href="javascript:void(0)">{{ $subMenu_title }}</a>
                                            </li>
                                        @endif
                                    @endforeach
                                </ul>
        ...
        @elsedesktop
            <div class="pull-right" style="position: absolute;left: 170px;top: 21px;">
        @enddesktop
            @if($WebsiteSettings->social_link6)
                <a href="{{ $WebsiteSettings->social_link6 }}" id="instagram-icon" class="pull-left" data-placement="top" title="Instagram" target="_blank">
                @desktop
                    <img alt="" src="{{ URL::to('/assets/frontend/img/instagram-icon.png') }}">
                @elsedesktop
                    <img alt="" src="{{ URL::to('/assets/frontend/img/instagram-icon.png') }}" style="transform: scale(0.7) translateX(30px)">
                @enddesktop
                </a>
            @endif
            @if($WebsiteSettings->social_link1)
                <a href="{{ $WebsiteSettings->social_link1 }}" id="facebook-icon" class="pull-left" data-placement="top" title="Facebook" target="_blank">
                @desktop
                    <img alt="" src="{{ URL::to('/assets/frontend/img/facebook-icon.png') }}">
                @elsedesktop
                    <img alt="" src="{{ URL::to('/assets/frontend/img/facebook-icon.png') }}" style="transform: scale(0.7);">
                @enddesktop
                </a>
            @endif
        </div>
        <style>
            ...
            #instagram-icon:hover {
                content: url('{{ URL::to('/assets/frontend/img/instagram-icon2.png') }}');
            }
            #facebook-icon:hover {
                content: url('{{ URL::to('/assets/frontend/img/facebook-icon2.png') }}');
            }
